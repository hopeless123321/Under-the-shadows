...
func execute(sender : Unit, recievers : Array[Unit]) -> void:
	...
	status_effect_dupl.initialization(recievers.front(), time_effected)
...
func reveal_result_action(sender : Unit, recievers : Array[Unit]) -> String:
	var text : String
	text += recievers.front().unit_property.forename + " Status apply: [img]" + status_effect_apply.mini_icon.resource_path + "[/img] for " + str(time_effected) + " turn(s)\n"
	for unit : Unit in recievers.slice(1):
